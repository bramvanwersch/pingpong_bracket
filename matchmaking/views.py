...
class MatchmakingView(BaseView):
    def get(self, request):
        ...
        waiting_matches = (
            MatchRequests.objects.filter(start__lt=current).filter(end__gt=current).filter(challenger=None)
        )
        request_data = get_match_request_data(waiting_matches)
        planned_matches = (
            MatchRequests.objects.filter(start__lt=current)
            .filter(end__gt=current)
            .exclude(challenger=None)
            .filter(Q(challenger=request.user) | Q(asker=request.user))
        )
...
class AddMatchRequest(BaseView):
    def post(self, request):
        data = request.POST
        MatchRequests.objects.create(
            start=data["from"], end=data["to"], asker=request.user, match_type=MatchRequests.MatchTypes.REQUEST
        )
...
class ChallengeRequest(BaseView):
    def get(self, request, name: str):
        ...
        MatchRequests.objects.create(
            asker=asker, challenger=request.user, start=start, end=end, match_type=MatchRequests.MatchTypes.CHALLENGE
        )
        return HttpResponseRedirect("/matchmaking/")
